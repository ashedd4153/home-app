...
federal_tax_savings = np.array([
    compute_tax(taxable_income, "Federal") - compute_tax(taxable_income - d, "Federal")
...
results = {
    ...
    "Mortgage Payments": [sum(mortgage_payments), 0],
    "Property Taxes": [sum(property_taxes), 0],
    "Maintenance Costs": [sum(maintenance_costs), 0],
    "HOA Fees": [sum(hoa_costs), 0],
    "Home Insurance": [sum(home_insurance), sum(renter_insurance_annual for _ in range(years))],
    "Less Federal Tax Savings": [-sum(federal_tax_savings), 0],
    "Less State & Local Tax Savings": [-sum(state_local_tax_savings), 0],
